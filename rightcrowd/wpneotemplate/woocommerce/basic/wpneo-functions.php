...
function campaign_listing_by_author_before_loop(){
    if (! empty($_GET['author'])) {
        echo '<h3>'.__('Other Companies from: ', 'wp-crowdfunding').' '.wpneo_crowdfunding_get_author_name_by_login(sanitize_text_field(trim($_GET['author']))).'</h3>';
    }
...
function wpneo_bio_campaign_action(){
    ...
    if( $author ){
        ...
        $html .= '<div class="wpneo-profile-name">'.wpneo_crowdfunding_get_author_name().'</div>';
            ...
            $html .= '<div class="wpneo-profile-campaigns">'.wpneo_crowdfunding_author_all_campaigns()->post_count.__( " Companies" , "wp-crowdfunding" ).' | '. wpneo_loved_campaign_count().__( " Watchlist" , "wp-crowdfunding" ).'</div>';
        $html .= '</div>';
