...
ob_start();
 ...
 if (username_exists($username)){
	...
	function login_form(){
	    ...
	    echo '
		    ...
		    <form action="' . $_SERVER['REQUEST_URI'] . '" method="post">
		    	<a href="'.esc_url(get_site_url()).'">
						<img class="center-image" src="'.get_template_directory_uri().'/images/logo.jpg" style="width: 55%" alt="" >
				</a>
	    ...
	    if(get_option('wpneo_registration_page_id')){
	    	echo "<p>".__('Don not have an account?','themeum')." <a href='".esc_url(get_permalink( get_option('wpneo_registration_page_id') ))."' title='".__('Sign Up','themeum')."'>".__('Sign Up','themeum')."</a></p>";	
	    }
